...
type Stream struct {
	...
	//
	// ★ 少了它就是一场自我强化的重发风暴：当时的 RTO 判据取**全局最快路径**的
	// 2×RTT+200ms、下界 500ms，而链路真实往返在拥塞时可以是几十秒。于是"还没收到 ACK"被当成
...
const (
	...
	// 吞吐基本归零；64 KiB 则又快又稳。
	//
	// ★★ 它与 ackThreshold 是**耦合**的，不能单独调小：下界一旦低于 ackThreshold，
	// 发送方在途永远凑不够触发对端回 ACK 的量，两边互相饿死，只能靠重传定时器
	// 一轮一轮往前挪。取 2×ackThreshold 是留一倍余量。
	//
	// 2026-08-22 实测过一次「把下界按共用路径的流数摊薄」——想法是常驻队列
	// 正比于流数（16 条流各 64 KiB = 1 MiB），本身没错，但摊到 16 KiB 就跌破了
	// ackThreshold，于是（pi5 → 192.168.20.239，千兆）：
	//
	//	16 条流   645.3 MiB/12s → 18.3 MiB/12s（35 倍），p99 22ms → 2.25s
	//	 4 条流   186.0 MiB/12s →  4.4 MiB/12s（42 倍），p99 2.1ms → 4.50s
	//	 1 条流    43.6 MiB/12s → 44.9 MiB/12s（不受影响，摊薄对 n=1 是恒等）
	//
	// 真要削常驻队列，两个常量必须**一起**往下走（代价是 ACK 帧变密），
	// 那是另一件事，不能只动这一个。TestInflightFloorCoversAckThreshold 钉着这条。
