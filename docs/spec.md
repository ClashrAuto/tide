...
**下面这组数字是修复前测的，仍保留作为记录**，但它测的是坏掉的 h3——
重新测量是让 h3 转正的前提之一。

实网测量（树莓派 5 ↔ x86，**双向** 5% 丢包，40 秒，4 条流）：

| 模式 | p50 | p90 | p99 | p99.9 | 路径建立次数 |
|---|---|---|---|---|---|
| 原生 QUIC | 1.05ms | 23ms | 306ms | 462ms | 2 |
| **h3** | **75ms** | **606ms** | **1.59s** | **2.18s** | **9** |

h3 模式在有损链路上**全面劣化**，而且关键信号是"路径建立 9 次"——h3 路径反复
建起来又死掉，流量最终全部退回 TCP，那条 TCP 的 srtt 被拖到 417ms。
在环回上这套东西是通的（两条测试都过），一到真实丢包就散架。

**原因尚未定位。** 已排除的：服务端日志零错误；不是握手失败（路径确实建起来了）。
待查的方向：h3 请求流在丢包下被 quic-go 提前终止、控制流上的探测帧不足以喂饱
静默计时器（`checkSilence` 8 秒）、或路径级 ctx 在某条边界上被提前撤销。

所以 h3 目前的状态是：**掩护那一半可用且有价值，数据面不可用于生产**。
`ClientConfig.H3` 保持默认 false。要让它成为默认，三件事缺一不可：
① 定位并修掉上面的路径churn；② 接 RFC 9297 的 HTTP Datagram（否则 UDP 是
可靠有序的，与 §9.1 相悖）；③ 重跑这张表并达到不劣于原生 QUIC。
