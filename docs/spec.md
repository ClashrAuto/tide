...
**默认仍关闭（`ClientConfig.H3` 默认 false）**，只剩一条原因：h3 模式下 UDP 走的是
可靠有序的控制流，而不是 §12.8 的 QUIC 数据报——RFC 9297 的 HTTP Datagram
（带 Quarter Stream ID 前缀）还没接。在那之前默认路径仍是原生 QUIC。
另外 h3 模式尚未做实网（丢包链路）测量，而原生 QUIC 那条是量过的。
