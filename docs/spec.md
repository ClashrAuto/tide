...
- ✅ **掩护那一半已完成并通过测试。** `Server.ServeH3` 让服务端对任何非 TIDE 的
  HTTP/3 客户端反向代理到掩护源站——实测探测方拿到的是 `200` + 源站真实的
  `Server:` 头与正文，不是沉默、也不是模拟。§12.6 的安全目标达成。
- ⚠️ **TIDE 自己的数据面在 h3 之上尚未跑通。** 直接调 `dialH3Path` 能建起路径，
  但经调度器驱动时路径建立后立刻死亡，原因未定位。
  因此 `ClientConfig.H3` **默认关闭**，默认仍走原生 QUIC 那条已实测的路径
  （p90 8.6ms → 1.67ms），不会因为这个未完成的特性回退。

实现过程中撞到并记下的三个坑，都属于"不写下来下次还会再踩"：
