...
**仍然待决的部分：残留差距。** 上述措施让 QUIC 端口"什么也给不出"，但**没有**让它
看起来像别的东西：探测方仍能确认「这个 UDP 端口上有一个会完成握手的 QUIC 服务端」。
要消掉这一层，需要给非 TIDE 的 QUIC 连接提供真实的 HTTP/3 掩护（Hysteria2 的
masquerade 是同一思路）。没有立刻做，是因为那不只是"接一个 h3 服务器"：
HTTP/3 的 SETTINGS 取值、QPACK 动态表行为、优先级处理都必须与被掩护的源站一致，
任何一处不同本身又是新的指纹。做半套比不做更糟——会把"一个普通 QUIC 端点"
变成"一个行为古怪的 HTTP/3 服务器"。
