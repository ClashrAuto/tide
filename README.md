...
- `cover` —— 掩护源站（nginx）。认证失败时 TIDE 把连接的**全部字节原样转发**给它，
  ...
  放在同一个 compose 网络里正合适。

  ⚠️ **`deploy/cover/index.html` 是占位页，部署前必须换掉。** 它在每一个用默认配置的
  部署上**逐字节相同**——扫一遍正文就能把所有默认部署一网打尽，而这正是掩护站点要防的
  事。而且一台在 443 上服务 HTTPS 的主机返回一张 "It works / Nothing to see here"，
  这件事本身就不像真站点。`tide-server` 启动时会检测这张页面并告警，
  换成你自己的内容（或让 nginx 反代到一个真实域名）之后告警自动消失。
...
spec §12 的八个条目**全部定稿**。最后一项 §12.6（QUIC 面的主动探测防御）也已落地：
服务端对任何非 TIDE 的 HTTP/3 客户端反向代理到掩护源站——探测方拿到的是源站的真实响应，
