...
| 期 | 内容 | 状态 |
|---|---|---|
| 一 | 前重后轻填充、裸帧模式协商 | 已实现（kTLS `splice` 尚未接入，见 spec §12.3） |
| 二 | 单次票据 0-RTT、信道绑定、混合后量子 | 已实现并实测 |
| 三 | 多路径调度（TCP ↔ QUIC） | 已实现；每路径单 QUIC 流，队头阻塞未完全消除（spec §12.5） |
...
一个反直觉但很重要的结论写在 spec §8.1：**TCP 路径的丢包率从应用层看恒为 0**，
所以 draft-00 里"丢包率超 2% 就迁移"的判据按字面实现永远不会触发。判据必须看 RTT。
